...
def create_question(jwt_token, title, question):
    url = "http://localhost:8080/api/v1/questions"
    headers = {
        "Authorization": jwt_token,
        "Content-Type": "application/json",
    }
    payload = json.dumps({"title": title, "question": question})

    return requests.request("POST", url, headers=headers, data=payload).json()


def create_question_get_id(jwt_token, title, question):
    response = create_question(jwt_token, title, question)
...
def update_question(jwt_token, question_id, title, question):
    url = f"http://localhost:8080/api/v1/questions/{question_id}"
